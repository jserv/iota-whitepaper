...
system. 
% to be conducted on a global scale. 
...
%used as a 
%backbone of ​iota​ (a cryptocurrency for Internet-of-Things industry). 
%Its main feature is that, instead of a blockchain,
%we use a \emph{tangle} (that is, a directed acyclic graph) 
%for storing transactions.
%This technology naturally succeeds the blockchain technology as its 
%next evolutionary step and comes out with features required 
%for micropayments conducted on a global scale. 
%
%In particular, one of contributions of this
%paper is a family of MCMC algorithms for selecting the sites
%of the tangle where a newly arrived transaction should
%be attached.

% In this paper we briefly describe the design of the 
% blockchainless cryptocurrency system \emph{iota}, 
% and make some calculations regarding the probabilistic 
% aspects of this system. 
% In particular, we discuss the question of system's security.
\end{abstract}
...
%, which prevent it to be used as a one and only global platform
% for cryptocurrencies.
%Among these drawbacks, an especially notable one is the impossibility
%of making micro-payments, which have increased importance for the 
%rapidly developing Internet-of-Things industry.
%Specifically, in the currently available systems one must
%pay a fee for making a transaction; 
%so, transferring a \emph{very} small amount just makes no sense
%since one would have also to pay the fee which is many 
%times larger. On the other hand, it is not easy to get rid
%of the fees since they serve as an incentive for the 
%creators of the blocks. It should be also observed that
%existing cryptocurrencies are heterogeneous systems
%with clear separation of roles (transaction issuers, transaction approvers). 
%Such systems create unavoidable discrimination of some of their
%elements which in turn creates conflicts and makes 
%all elements spend resources on conflict resolution.
%All this justifies a search for solutions essentially different from
%the blockchain technology, on which the Bitcoin and many
...
%a blockchainless approach,
%which is currently being implemented in \emph{iota}~\cite{iota},
%recently designed as
%  a cryptocurrency for the Internet-of-Things industry. 
  
% An important part of our proposal is a novel cryptographic
% signing algorithm, that will be described in detail in another
% whitepaper~\cite{iota_crypto}. 
% We, however, focus more on general features and problems/difficulties
% that arise when one attempts to get rid of the blockchain,
% than on concrete details of iota's implementation. 
% Of course, any theoretical analysis would be
%  incomplete without feedback from a working version
% of such a system; we hope that soon such a feedback would be 
% available.
%We, however, focus more on general features and problems/difficulties
%that arise when one attempts to get rid of the blockchain,
%than on concrete details of iota's implementation. 
...
% (i.e., the tangle graph is the ledger for storing transactions).
%Its 
...
% (no other tokens will be created),
...
% and there no mining in the sense ``miners receive
% monetary rewards''.
...
% and do not approve, directly or indirectly,
%conflicting transactions

As a transaction 
% gets more and more (direct or indirect) 
receives additional 
...
% assumption, especially in the context of IoT, where nodes
% are specialized chips with pre-installed firmware)
...
%\footnote{see Section~\ref{s_parasite}}.
 \item 
 %It 
...
% (e.g., it needs to find a nonce such that the hash of that 
% nonce together with some data from the approved transactions 
% has a particular form, for instance, has at
% least some fixed number of zeros in front).
% \item comment on computation and propagation times??
\end{itemize}
It is important to observe that
%, in general, 
the iota network is asynchronous.
% have an asynchronous network.
In general,
% so that 
...
% i.e., 
% transactions issued according to the protocol
} transactions
have the right
to be in the ledger,
...
% (that is, eventually not indirectly approved by incoming 
...
if
%, after 100 runs of the tip selection algorithm,
...
% A reasonable rule of thumb for considering a transaction
% confirmed can be 
% the following: first wait until
% the adaptation period is over (see Section~\ref{s_cum_grow};
% essentially, that means that ``almost'' all newly arriving
% transactions indirectly approve our transaction), 

Let us also comment on the following question (cf.\ \cite{red_balloons}):
what motivates the nodes to propagate transactions? 
% In fact, nodes do not 
% have motivation 
%% to not 
%to not propagate in our system. (this is a double negative, makes wording confusing)
...
e.g.~\cite{SZ_SPECTRE}.
%which appeared after the first version of this paper
We 
% mention 
